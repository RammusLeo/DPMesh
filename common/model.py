...
class Model(nn.Module):
    def __init__(self, backbone, vposer, cfg):
        super(Model, self).__init__()
        self.backbone = backbone
        # self.backbone.requires_grad_(False)
        # self.unet_lora_params, self.train_names = inject_trainable_lora(self.backbone)
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        img = self.backbone(x = inputs['img'], control=input_heatmap,context=context)
        ...
        if mode == 'train':
            ...
            for layer_idx in range(L):
                ...
                loss['smpl_pose_{}'.format(layer_idx)] = 2 * self.param_loss(pose_param[layer_idx], targets['pose_param'], meta_info['fit_param_valid'] * meta_info['is_valid_fit'][:, None])
                loss['smpl_shape_{}'.format(layer_idx)] =  2 * self.param_loss(shape_param[layer_idx], targets['shape_param'], meta_info['is_valid_fit'][:, None])
                loss['body_joint_proj_{}'.format(layer_idx)] = self.coord_loss(joint_proj[layer_idx], targets['orig_joint_img'][:, :, :2], meta_info['orig_joint_trunc'])
                loss['body_joint_cam_{}'.format(layer_idx)] = 2 * self.coord_loss(joint_cam[layer_idx], targets['orig_joint_cam'], meta_info['orig_joint_valid'] * meta_info['is_3D'][:, None, None])

            return loss

        else:
            # test output
            out_info = {'img_path': meta_info['img_path']}
            ...
            out['input_joint_vis'] = targets['input_joint_vis']
...
def get_model(joint_num, mode, cfg):
    ...
    # for k,v in cldm.named_parameters():
    #     print(k)
    # cldm.sd_model.load_state_dict(
    #     load_state_dict('./common/vpd/control_sd15_openpose.pth', location='cuda'))
    vposer = Vposer()
    ...
    model = Model(backbone = cldm, vposer = vposer, cfg=cfg)
    return model
