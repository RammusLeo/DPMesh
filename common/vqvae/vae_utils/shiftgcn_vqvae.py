...
from .graph import ShiftGCN_Encoder, ShiftGCN_Decoder
...
class GCNVQVAE(nn.Module):
    def __init__(self,
        ...
        self.encoder_gcn = ShiftGCN_Encoder(graph=self.graph)
        # self.decoder_gcn = ShiftGCN_Decoder(graph=self.graph)
    ...
    def forward(self, pose):
        ...
        #------------Encode--------------
        encode_feat = self.start_embed(pose)    # B, 64, 23
        ...
        encode_feat = self.encoder_gcn(encode_feat)
        ...
        # decode_feat = self.decoder_gcn(decode_feat).transpose(2,1)
