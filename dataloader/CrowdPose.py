...
class CrowdPose(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == 'train':
            ...
            inputs = {'img': img, 
                      'joints': input_crowdpose_joint_img[:, :2], 
                      'joints_mask': crowdpose_joint_trunc}
            targets = {'orig_joint_img': crowdpose_joint_img, 
                       'fit_joint_img': smpl_joint_img, 
                       'orig_joint_cam': crowdpose_joint_cam, 
                       'fit_joint_cam': smpl_joint_cam, 
                       'pose_param': smpl_pose, 
                       'shape_param': smpl_shape}
            meta_info = {'cliff_focal_length': cliff_focal_length,
                         'orig_joint_valid': crowdpose_joint_valid, 
                         'orig_joint_trunc': crowdpose_joint_trunc, 
