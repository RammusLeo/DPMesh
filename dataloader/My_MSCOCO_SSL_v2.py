...
from common.utils.preprocessing import load_img, process_bbox, augmentation, compute_iou, load_img_from_lmdb,addocc
# from utils.vis import vis_bbox, render_mesh
from common.utils.transforms import world2cam, cam2pixel, pixel2cam, transform_joint_to_other_db




class My_MSCOCO_SSL_v2(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.img_path = osp.join(cfg.root_dir, 'data', 'MSCOCO', 'images')
        self.annot_path = osp.join(cfg.root_dir, 'data', 'MSCOCO', 'annotations')
        self.annot_path_new = osp.join('data', 'annotations', 'mscoco3.pkl')
        self.rootnet_output_path = osp.join(cfg.root_dir, 'data', 'MSCOCO', 'rootnet_output', 'bbox_root_coco_output.json')
        self.fitting_thr = 3.0 # pixel in cfg.output_hm_shape space

        self.img_file = osp.join(cfg.metro_dir, 'coco_smpl', 'train.img.tsv')
        self.hw_file = osp.join(cfg.metro_dir, 'coco_smpl', 'train.hw.tsv')
        self.linelist_file = osp.join(cfg.metro_dir, 'coco_smpl','smpl', 'train.linelist.tsv')
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == 'train':
            ...
            import pdb; pdb.set_trace()
            """
            input_coco_joint_img[:, 0] = input_coco_joint_img[:, 0] / cfg.input_img_shape[1] * cfg.input_hm_shape[1]
            input_coco_joint_img[:,1] = input_coco_joint_img[:,1] / cfg.input_img_shape[0] * cfg.input_hm_shape[0]
            ...
            teacher_coco_joint_img = coco_joint_img.copy()
            teacher_coco_joint_img[:, 0] = coco_joint_img[:, 0] / cfg.output_hm_shape[1] * cfg.input_hm_shape[1]
            teacher_coco_joint_img[:, 1] = coco_joint_img[:, 1] / cfg.output_hm_shape[0] * cfg.input_hm_shape[0]
            # images = {'img_teacher': img_teacher, 'img_student': img_student}
            # masks = {'mask_teacher': joint_mask, 'mask_student': joint_mask_student}
            # inputs = {'joints': input_coco_joint_img[:, :2]}
            teacher_inputs = {'img': img_teacher, 'joints_mask': joint_mask, 'joints': teacher_coco_joint_img}
            student_inputs = {'img': img_student, 'joints_mask': joint_mask_student, 'joints': input_coco_joint_img}
            targets = {
            ...
            meta_info = {
                'cliff_focal_length': cliff_focal_length,
            ...
            inputs = {'img': img_student, 'joints': coco_joint_img, 'joints_mask': coco_joint_trunc}
            targets = {}
            meta_info = {'bbox': bbox}
