...
class CrowdPose_SSL(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == 'train':
            ...
            mask_prop = np.random.rand(crowdpose_joint_trunc.shape[0],crowdpose_joint_trunc.shape[1])  # B, N, 1
            mask = mask_prop < 0.3 # 10% of joints are masked
            joint_mask_student = crowdpose_joint_trunc * (1 - mask.astype(np.float32))
            ...
            # masks = {'mask_teacher': crowdpose_joint_trunc, 'mask_student': joint_mask_student}
            # inputs = {'joints': input_crowdpose_joint_img[:, :2]}
            teacher_inputs = {'img': img_teacher, 'joints_mask': crowdpose_joint_trunc, 'joints': crowdpose_joint_img}
            student_inputs = {'img': img_student, 'joints_mask': joint_mask_student, 'joints': input_crowdpose_joint_img}
