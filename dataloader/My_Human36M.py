...
class My_Human36M(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        print('='*20, 'Human36M', '='*20)
        self.transform = transform
        self.data_split = data_split
        self.img_dir = osp.join(cfg.root_dir, 'data', 'Human36M', 'images')
        self.img_file = osp.join(cfg.metro_dir, 'human3.6m', 'train.img.tsv')
        self.img_tsv = self.get_tsv_file(self.img_file)
        self.hw_file = osp.join(cfg.metro_dir, 'human3.6m', 'train.hw.tsv')
