...
from common.utils.vis import vis_keypoints, vis_mesh, save_obj, vis_keypoints_with_skeleton, vis_bbox,vis_keypoints_with_skeleton_fortrain
from common.utils.openpose_utils import OPENJOINTSNAME,OPENSKELETON,OPENCOLOR

import transforms3d


class MuCo_v2_skele(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        print('='*20, 'MuCo', '='*20)
        self.transform = transform
        self.data_split = data_split
        self.img_dir = osp.join(cfg.root_dir, 'data', 'MuCo','images')
        self.annot_path = osp.join(cfg.root_dir, 'data', 'MuCo', 'annotations','MuCo-3DHP.json')
        self.annot_path_new = osp.join('data', 'annotations', 'muco4.pkl')
        self.smpl_param_path = osp.join(cfg.root_dir, 'data', 'MuCo', 'annotations','smpl_param.json')
    ...
    def __getitem__(self, idx):
        ...
        cv2.waitKey(1)
        """
        input_muco_joint_img[:, 0] = input_muco_joint_img[:, 0] / cfg.input_img_shape[1] * cfg.input_hm_shape[1]
        input_muco_joint_img[:, 1] = input_muco_joint_img[:, 1] / cfg.input_img_shape[0] * cfg.input_hm_shape[0]
        input_kp_trunc = transform_joint_to_other_db(muco_joint_trunc, self.joints_name, self.openpose_joints_name)
        input_kp_img = transform_joint_to_other_db(input_muco_joint_img, self.muco_joints_name, self.openpose_joints_name)
        ...
        inputs = {'img': img, 
                #   'joints': input_muco_joint_img[:, :2], 
                #   'joints_mask': muco_joint_trunc,
