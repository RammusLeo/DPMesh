...
import logging
import code
from common.utils.comm import get_world_size
from dataloader.my_dataset import (MeshTSVDataset, MeshTSVYamlDataset)
...
def make_data_loader(args, yaml_file, is_distributed=True, 
    ...
    if is_train==True:
        shuffle = True
        images_per_gpu = args.train_batch_size
        images_per_batch = images_per_gpu * get_world_size()
        iters_per_batch = len(dataset) // images_per_batch
        num_iters = iters_per_batch * args.end_epoch
        logger.info("Train with {} images per GPU.".format(images_per_gpu))
        logger.info("Total batch size {}".format(images_per_batch))
        logger.info("Total training steps {}".format(num_iters))
    else:
        shuffle = False
        images_per_gpu = args.test_batch_size
        num_iters = None
        start_iter = 0
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, num_workers=args.num_thread, batch_sampler=batch_sampler,
        pin_memory=True,
    )
    return data_loader,iters_per_batch, batch_sampler
